...
export class ShardConsumer {
    ...
    private async processRecordBatch(recordsResponse: GetRecordsCommandOutput): Promise<void> {
        ...
        const tasks = recordsResponse.Records!
            .map((record) => {
                try {
            ...
            .filter((task): task is BaseTask<any> => task !== null);

        if (tasks.length > 0) {
            logger.info(`${logPrefix} Processing ${tasks.length} tasks`);

            try {
                logger.debug(`${logPrefix} Calling processor function...`);

                await Promise.race([
                    processor(`${streamId}:${shardId}`, tasks),
